...
const TroubleshootingItem = ({ issue, solution, matches }) => {
  return (
    <details className="card p-3 mb-3">
      <summary className="fw-semibold d-flex justify-content-between align-items-center">
        <span>{highlightMatch(issue, matches, "issue")}</span>
        <i className="fas fa-chevron-down ms-2"></i>
      </summary>
      <p className="mt-3 mb-0">{highlightMatch(solution, matches, "solution")}</p>
    </details>
  );
};
